...
/**
 * generate-reels.mjs
 * Generates 50 .webm video files from reel templates using Puppeteer.
 * Each reel is rendered as a standalone HTML page, then recorded via CDP screencast.
 */
...
// ─── Reel data (inline to avoid TS import issues) ────────────
const REELS = [
...
// ─── Generate standalone HTML for a reel ─────────────────────
function generateReelHTML(reel) {
  const sceneCount = reel.scenes.length;
  const sceneDuration = (reel.duration * 1000) / sceneCount;

  const scenesHTML = reel.scenes
    .map((s, i) => {
      const startPct = (i / sceneCount) * 100;
      const endPct = ((i + 1) / sceneCount) * 100;
      const gradientStops = s.bg
        .map((c, ci) => `${c} ${(ci / (s.bg.length - 1)) * 100}%`)
        .join(", ");

      let effectCSS = "";
      switch (s.effect) {
        case "zoom":
          effectCSS = `from { transform: scale(0.3); opacity: 0; } to { transform: scale(1); opacity: 1; }`;
          break;
        case "slide-up":
          effectCSS = `from { transform: translateY(120px); opacity: 0; } to { transform: translateY(0); opacity: 1; }`;
          break;
        case "fade-scale":
          effectCSS = `from { transform: scale(0.8); opacity: 0; } to { transform: scale(1); opacity: 1; }`;
          break;
        case "spin-in":
          effectCSS = `from { transform: rotate(-180deg) scale(0); opacity: 0; } to { transform: rotate(0) scale(1); opacity: 1; }`;
          break;
        case "bounce-in":
          effectCSS = `from { transform: scale(0.1); opacity: 0; } 60% { transform: scale(1.1); } to { transform: scale(1); opacity: 1; }`;
          break;
        case "glitch":
          effectCSS = `from { transform: translateX(-20px); opacity: 0; filter: blur(4px); } to { transform: translateX(0); opacity: 1; filter: blur(0); }`;
          break;
        case "float":
          effectCSS = `from { transform: translateY(40px); opacity: 0; } to { transform: translateY(0); opacity: 1; }`;
          break;
        case "shimmer":
          effectCSS = `from { opacity: 0; } to { opacity: 1; }`;
          break;
        case "ripple":
          effectCSS = `from { transform: scale(0); opacity: 0; border-radius: 50%; } to { transform: scale(1); opacity: 1; border-radius: 0; }`;
          break;
        default:
          effectCSS = `from { opacity: 0; } to { opacity: 1; }`;
      }
      ...
      .scene-${i} {
        ...
        color: ${s.textColor || "#191510"};
        opacity: 0;
        animation: scene-in-${i} ${sceneDuration}ms linear ${i * sceneDuration}ms forwards;
      }
      @keyframes scene-in-${i} {
        ...
        15% { opacity: 1; transform: ${
          s.effect === "zoom" ? "scale(1)" :
          s.effect === "slide-up" ? "translateY(0)" :
          s.effect === "bounce-in" ? "scale(1.1)" :
          s.effect === "spin-in" ? "rotate(10deg) scale(1.05)" :
          "translateY(0)"
        }; }
        25% { opacity: 1; transform: ${
          s.effect === "bounce-in" ? "scale(1)" :
          s.effect === "spin-in" ? "rotate(0) scale(1)" :
          "none"
        }; }
        90% { opacity: 1; }
        100% { opacity: 0; }
  ...
  const scenesContent = reel.scenes
    .map(
      (s, i) => `
      <div class="scene-${i}">
        ...
        <div class="progress-bar">
          <div class="progress-fill" style="animation: progress-${i} ${sceneDuration}ms linear ${i * sceneDuration}ms forwards;"></div>
        </div>
      </div>`
    )
    .join("\n");

  const progressAnimations = reel.scenes
    .map(
      (_, i) => `
      @keyframes progress-${i} {
        from { width: ${(i / sceneCount) * 100}%; }
        to { width: ${((i + 1) / sceneCount) * 100}%; }
      }`
    )
    .join("\n");
...
<html><head><meta charset="utf-8"><style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .viewport { position: relative; width: 1080px; height: 1920px; overflow: hidden; }
  ${scenesHTML}
  .emoji { font-size: 140px; margin-bottom: 20px; animation: float-emoji 2s ease-in-out infinite; }
  @keyframes float-emoji { 0%,100% { transform: translateY(0); } 50% { transform: translateY(-15px); } }
  .title {
    font-family: Georgia, serif; font-size: 90px; font-weight: bold;
    text-align: center; line-height: 1.15; white-space: pre-line;
    padding: 0 40px;
  }
  .subtitle {
    font-family: Arial, sans-serif; font-size: 38px; margin-top: 12px;
    opacity: 0.8; text-align: center; padding: 0 60px;
  }
  .brand {
    position: absolute; bottom: 60px; left: 0; right: 0;
    text-align: center; font-family: Arial, sans-serif;
    font-size: 22px; letter-spacing: 3px; opacity: 0.4;
  }
  .progress-bar {
    position: absolute; bottom: 0; left: 0; right: 0; height: 8px;
    background: rgba(0,0,0,0.2);
  }
  .progress-fill { height: 100%; background: rgba(255,255,255,0.8); width: 0; }
  ${progressAnimations}
</style></head><body>
<div class="viewport">${scenesContent}</div>
</body></html>`;
}

// ─── Main: render all reels ──────────────────────────────────
async function main() {
  if (!existsSync(OUT_DIR)) mkdirSync(OUT_DIR, { recursive: true });

  console.log(`\n🎬 Generating ${REELS.length} reel videos...\n`);
  ...
  for (const reel of REELS) {
    const fileName = `reel-${String(reel.id).padStart(2, "0")}-${reel.name.toLowerCase().replace(/[^a-z0-9]+/g, "-")}.webm`;
    ...
    try {
      ...
      const html = generateReelHTML(reel);
      await page.setContent(html, { waitUntil: "domcontentloaded" });

      // Use CDP screencast to capture the reel as it animates
      ...
      // Wait for all scenes to play through
      const totalDuration = reel.duration * 1000 + 500; // extra 500ms buffer
      ...
      if (frameData.length === 0) {
        console.log(`  ⚠️  #${reel.id} ${reel.name} — no frames captured, skipping`);
        failCount++;
        continue;
      }

      // Write raw JPEG frames as a tar-like sequence for later conversion
      // For now, save as a series of numbered frames
      const framesDir = join(OUT_DIR, `reel-${String(reel.id).padStart(2, "0")}-frames`);
      if (!existsSync(framesDir)) mkdirSync(framesDir, { recursive: true });

      for (let i = 0; i < frameData.length; i++) {
        writeFileSync(join(framesDir, `frame-${String(i).padStart(4, "0")}.jpg`), frameData[i].data);
      }

      console.log(
        `  ✅ #${String(reel.id).padStart(2, "0")} ${reel.name} — ${frameData.length} frames captured`
      );
      successCount++;
    } catch (err) {
      console.log(`  ❌ #${reel.id} ${reel.name} — ERROR: ${err.message}`);
      failCount++;
    }
  }
  ...
  console.log(`\n✨ Done! ${successCount} reels generated, ${failCount} failed`);
  console.log(`📁 Output: ${OUT_DIR}`);
  console.log(
    `\n⚠️  Frames are saved as JPEG sequences. Run "node scripts/encode-reels.mjs" to encode them as .webm videos.`
  );
}
